...
import { Plus, MessageSquare, CheckCircle2, Clock, Trash2, Sparkles, Loader2 } from 'lucide-react';
...
export const ConversationList: React.FC<ConversationListProps> = ({
  ...
  return (
    <div id="conversation-list-container" className="flex flex-col h-full bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden">
      ...
      <div className="p-4 border-b border-slate-100 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-[11px] text-slate-500">Private AI-assisted dialogue</p>
        </div>
        ...
        <button
          ...
          className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-semibold shadow-xs transition cursor-pointer shrink-0"
        >
      ...
      {deleteError && (
        <div className="p-3 bg-rose-50 border-b border-rose-200 text-rose-700 text-xs flex items-center justify-between gap-2">
                ...
                {activeConversations.map((conv) => {
                  ...
                  return (
                    <div
                      ...
                      className={`group p-3 rounded-xl border transition cursor-pointer flex items-start justify-between gap-2 ${
                        isSelected
                          ? 'bg-slate-100/90 border-slate-400 text-slate-900 shadow-xs'
                    ...
                    >
                      <div className="space-y-1 min-w-0 flex-1">
                        ...
                        <p className="text-[11px] text-slate-500 line-clamp-1">In progress · Tap to continue</p>
                      </div>
                      ...
                      <button
                        ...
                        className="opacity-70 hover:opacity-100 group-hover:opacity-100 p-1.5 text-slate-400 hover:text-rose-600 rounded-md transition cursor-pointer"
                      >
                        {deletingId === conv.id ? (
                    ...
                    >
                      <div className="space-y-1 min-w-0 flex-1">
                        ...
                        {conv.summary ? (
                          ...
                          <p className="text-[11px] text-slate-400">Completed session</p>
                        )}
                      </div>
