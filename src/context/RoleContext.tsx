...
export const RoleProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchRoleFromBackend = useCallback(async () => {
    ...
    try {
      ...
      if (!token) {
        ...
        console.warn('[RoleContext] Unable to acquire ID token. Defaulting to user role.');
      ...
      if (!response.ok) {
        ...
        console.warn(`[RoleContext] Role resolution failed (HTTP ${response.status}). Defaulting to user role.`);
        setRole(DEFAULT_USER_ROLE);
        setRoleResolved(true);
        setLoading(false);
        return;
      ...
      if (data && typeof data.role === 'string' && (data.role === 'user' || data.role === 'admin')) {
        setRole(data.role as UserRole);
      } else {
        // Invalid response shape — fail to default user role
        console.warn('[RoleContext] Invalid role response shape. Defaulting to user role.');
        setRole(DEFAULT_USER_ROLE);
      }
      ...
      console.warn('[RoleContext] Role resolution error. Defaulting to user role.', err);
      setRole(DEFAULT_USER_ROLE);
      setRoleResolved(true);
  ...
  // Resolve role when auth state changes (production)
  useEffect(() => {
    if (isDemoSession) {
      // Demo mode: use demo role from DemoContext
      setRole(demoRole || DEFAULT_USER_ROLE);
      setRoleResolved(true);
      return;
    }
