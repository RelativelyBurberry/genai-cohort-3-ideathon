...
interface DemoContextType {
  ...
  updateDemoJournalEntry: (entryId: string, input: UpdateJournalEntryInput) => Promise<void>;
...
function loadDemoWorkspace(): DemoWorkspace {
  try {
    ...
    if (stored) {
      ...
      return {
        ...
        conversations: parsed.conversations.map((c: any) => ({
          ...
          updatedAt: c.updatedAt ? Timestamp.fromDate(new Date(c.updatedAt)) : null,
          summaryUpdatedAt: c.summaryUpdatedAt ? Timestamp.fromDate(new Date(c.summaryUpdatedAt)) : null,
        })),
        messages: Object.fromEntries(
          Object.entries(parsed.messages).map(([convId, msgs]: [string, any]) => [
            convId,
            msgs.map((m: any) => ({
              ...m,
              createdAt: m.createdAt ? Timestamp.fromDate(new Date(m.createdAt)) : null,
            })),
          ])
        ),
      };
    ...
    console.warn('[DemoContext] Failed to load demo workspace from localStorage:', err);
  }
...
function saveDemoWorkspace(workspace: DemoWorkspace): void {
  try {
    ...
    console.warn('[DemoContext] Failed to save demo workspace to localStorage:', err);
  }
}
...
export const DemoProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const startDemoSession = useCallback(() => {
    if (!isDemoMode) {
      console.warn('[DemoContext] Cannot start demo session: VITE_DEMO_MODE is not enabled.');
      return;
    }
  ...
  // Journal adapters
  const createDemoJournalEntry = useCallback(async (input: CreateJournalEntryInput): Promise<string> => {
  ...
  const updateDemoJournalEntry = useCallback(async (entryId: string, input: UpdateJournalEntryInput): Promise<void> => {
    setWorkspace(prev => ({
      ...prev,
  ...
  const deleteDemoJournalEntry = useCallback(async (entryId: string): Promise<void> => {
    setWorkspace(prev => ({
      ...prev,
  ...
  const createDemoConversation = useCallback(async (title: string): Promise<string> => {
    const id = generateDemoId();
    const now = Timestamp.now();
    const conv: Conversation = {
      id,
      title: title || `Reflection Session · ${new Date().toLocaleDateString()}`,
      summary: null,
      status: 'active',
      createdAt: now,
      updatedAt: now,
  ...
  const deleteDemoConversation = useCallback(async (conversationId: string): Promise<void> => {
    setWorkspace(prev => {
  ...
  const getDemoMessages = useCallback((conversationId: string): ReflectionMessage[] => {
    return workspace.messages[conversationId] || [];
  }, [workspace.messages]);
  
  const sendDemoMessage = useCallback(async (conversationId: string, content: string): Promise<void> => {
    ...
    setWorkspace(prev => ({
      ...
      messages: {
        ...prev.messages,
        [conversationId]: [...(prev.messages[conversationId] || []), userMsg, assistantMsg],
      },
